...
#include <chrono>
#include <cstdint>
#include "Hasher-ChatGPT.hpp"
#include "../sha256.hpp"

using namespace std;

string to_hex(const array<uint8_t,16>& h){
    static const char* H="0123456789abcdef";
    string s; s.resize(32);
    for (int i = 0; i < 16; ++i) {
        s[2*i]   = H[(h[i] >> 4) & 0xF];
        s[2*i+1] = H[h[i] & 0xF];
    }
...
double hamming_distance_bits(const std::array<uint8_t,16>& a,
                             const std::array<uint8_t,16>& b) {
    int d = 0;
    for (int i = 0; i < 16; ++i) {
        uint8_t x = uint8_t(a[i] ^ b[i]);
        // popcount(x) without intrinsics
        for (int bit = 0; bit < 8; ++bit) d += (x >> bit) & 1;
    }
    // 16 bytes = 128 bits
    return 100.0 * double(d) / 128.0;
}

double hamming_distance_hex(const std::array<uint8_t,16>& a,
                            const std::array<uint8_t,16>& b) {
    int d = 0;
    for (int i = 0; i < 16; ++i) {
        d += (((a[i] >> 4) & 0x0F) != ((b[i] >> 4) & 0x0F)); // high nibble
        d += ((a[i] & 0x0F) != (b[i] & 0x0F));               // low nibble
    }
    // 16 bytes = 32 hex chars (nibbles)
    return 100.0 * double(d) / 32.0;
}
